...
function formatAge(minAge, maxAge) {
  ...
  if (minAge == null) return `до ${maxAge} лет`;
  if (maxAge == null) return `от ${minAge} лет`;
...
export default function ClubPageClient({ club }) {
  ...
  return (
    // ВАЖНО: Добавлено h-[calc(100vh-72px)] и overflow-y-auto, чтобы страница скроллилась
    // при body { overflow: hidden }, и padding-bottom для мобилок
      ...
      <div className="max-w-[1100px] mx-auto px-4 py-6 lg:py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8 items-start">
          ...
          <div className="flex flex-col gap-6 min-w-0">
            ...
            <div className="flex flex-col gap-3">
               ...
               <div className="flex items-center gap-2 text-gray-500 text-sm">
                 <svg className="shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
            ...
            <div className="relative aspect-video w-full overflow-hidden rounded-2xl bg-gray-200 shadow-sm">
              ...
              {photos.length > 1 && (
                <>
                  <button onClick={() => setActivePhotoIndex(prev => prev === 0 ? photos.length - 1 : prev - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white transition-all">‹</button>
                  <button onClick={() => setActivePhotoIndex(prev => prev === photos.length - 1 ? 0 : prev + 1)} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white transition-all">›</button>
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                    {photos.map((_, idx) => (
                      <div key={idx} className={`w-2 h-2 rounded-full transition-all ${idx === activePhotoIndex ? 'bg-white w-4' : 'bg-white/50'}`} />
                    ))}
                  </div>
                </>
              )}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              {priceText && (
                  <div className="lg:hidden mb-6 pb-6 border-b border-gray-100">
                      ...
                      <div className="text-3xl font-bold text-gray-900">{priceText}</div>
                  </div>
              )}

              <h2 className="text-xl font-bold mb-4 text-gray-900">О кружке</h2>
            ...
            {schedules.length > 0 && (
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <h2 className="text-xl font-bold mb-4 text-gray-900">Расписание занятий</h2>
          ...
          <aside className="hidden lg:flex flex-col gap-4 sticky top-6 self-start">
            ...
            {priceText && (
               <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                 <div className="text-sm text-gray-500 mb-1 font-medium text-[#69AFDF]">Стоимость обучения</div>
                 <div className="text-2xl font-extrabold text-gray-900">{priceText}</div>
                 {club.priceNotes && <div className="text-xs text-green-600 mt-2 bg-green-50 inline-block px-2 py-1 rounded">{club.priceNotes}</div>}
               </div>
            )}
            ...
            {club.socialLinks && Object.values(club.socialLinks).some(Boolean) && (
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                <div className="text-xs font-bold text-gray-400 uppercase mb-3 tracking-wider">Мы в соцсетях</div>
                <div className="flex flex-col gap-2">
                  {club.socialLinks.vk && (
                    <a href={club.socialLinks.vk} target="_blank" className="flex items-center gap-2 text-sm font-medium text-[#0077FF] hover:underline p-2 hover:bg-blue-50 rounded-lg transition-colors">
                      ВКонтакте
                    </a>
                  )}
                  {club.socialLinks.telegram && (
                    <a href={club.socialLinks.telegram} target="_blank" className="flex items-center gap-2 text-sm font-medium text-[#24A1DE] hover:underline p-2 hover:bg-blue-50 rounded-lg transition-colors">
                      Telegram
                    </a>
                  )}
                </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 lg:hidden z-50 flex gap-3 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
           <a href={club.phone ? `tel:${club.phone}` : '#'} className="flex-1 bg-[#69AFDF] text-white font-bold rounded-xl flex items-center justify-center py-3">
             {club.phone ? 'Позвонить' : 'Нет номера'}
           </a>
           <button onClick={() => alert('В разработке')} className="w-14 h-14 border-2 border-gray-200 rounded-xl flex items-center justify-center text-gray-500">
             <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
           </button>
        </div>
